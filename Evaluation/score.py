...
################################################################
#REVISION 0.1 AUTHOR : SRIVIDHYA BALAJI
#CHANGES :  DEBUGGED CODE TO GIVE INPUTS AND PROCESS BYTE STREAM 
################################################################
...
if __name__ == "__main__":
	...
	score_file = open(output_dir, 'wb')

	try:
		# Compute "real scores"
		# Get all the solution files from the solution directory
		solution_name = ref_file#os.path.join(input_dir, 'ref', 'valid.solution.csv')
		submission_name = input_dir#os.path.join(input_dir, 'res', 'submission.txt')
		...
		with open(solution_name, 'rb') as real_answer, \
			...
			while True:
				try:
					...
					current_ans_split = current_ans.split(b';')
					current_sub_split = current_sub.split(b' ')
					...
					# Increase counters
					current_search = (current_ans_split[3] == b"TRUE")
					if current_search:
						counter_search += 1
					else:
						counter_category += 1
				...
				q = current_ans_split[1]
				scores_raw_string = current_ans_split[4].replace(b'""', b'"')
				...
				sub_items = current_sub_split[1].replace(b"\n", b"").split(b',')
				...
				for i, sub_item in enumerate(sub_items):
					if sub_item not in counted_items:
						...
						dcg += ((2. ** ans_json.get(sub_item.decode('utf-8'),0)) - 1.) / log2(i + 2.)

				idcg_raw = current_ans_split[5].replace(b',', b'.')
				idcg = float(idcg_raw)
				...
				#score_file.write(bytes(" %0.6f\n "%NDCG_category ,'utf8'))
				
			NDCG_search = (NDCG_search / counter_search) #if counter_search != 0 else 0.0
			NDCG_category = NDCG_category / counter_category #if counter_category != 0 else 0.0
			NDCG_total = NDCG_category * 0.8 + NDCG_search * 0.2
			score_file.write(bytes("search_NDCG: %0.6f\n" % NDCG_search,'utf8'))
			score_file.write(bytes("category_NDCG: %0.6f\n" % NDCG_category,'utf8'))
			score_file.write(bytes("avg_NDCG: %0.6f\n" % NDCG_total,'utf8'))
	except:
		score_file.write(bytes("search_NDCG: 0\n",'utf8'))
		score_file.write(bytes("category_NDCG: 0\n",'utf8'))
